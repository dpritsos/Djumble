...
cdef class VBuffer:
    ...
    def __cinit__(self, Py_ssize_t vbz_size, Py_ssize_t buff_size):
        ...
        self.type = type
        if vbz_size == 1:
            self.buffer = <double*> malloc(self.buff_size*sizeof(double))
        else:
            pass
    ...
    def __getbuffer__(self, Py_buffer *buffer, int flags):
        ...
        buffer.len = self.buff_size*self.buff_size*itemsize   # product(shape) * itemsize
        buffer.ndim = 2
        buffer.obj = self
        buffer.readonly = 0
