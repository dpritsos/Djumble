...
\While{$I$ iterations not reached}{
	...
	\textbf{2. M-step }\\
		\hspace{.05\textwidth} \textbf{a.} Re-calculating $\mathbf{M}^{i+1}=(\mathbf{\mu}_{1}^{i+1},...,\mathbf{\mu}_{k}^{i+1})$ to minimize $Jobj$ objective function;\\
		\hspace{.05\textwidth} \textbf{b.} Re-estimating parameters $\mathbf{A}^{i+1}$;\\

	\eIf{$Jobj$ min threshold has been reached}{
		Breaking the loop and Ending the Clustering\;
	}{
